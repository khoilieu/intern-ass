...
def webhook_trigger():
    ...
    if not is_valid_event:
        log_event_status("failure", error_message, api_key, log_type="webhook")
    ...
    try:
        ...
        log_event_status("failure", f"Failed to store payload: {str(e)}", api_key, log_type="webhook")
        return jsonify({"message": "Failed to store payload"}), 500

    # Step 3: Log Success
    log_event_status("success", "Webhook event processed successfully", api_key, log_type="webhook")
